...
public class LuceneCreateIndexUtil {
    ...
    public static void createIndexbyPath(String path) {
        try {
            Directory directory = FSDirectory.open(new File("F:\\lucene\\test"));
    ...
    private static void creatIndex(Directory directory) {
        ...
        try {
            ...
            for (int i = 0; i < MockDataUtil.IDS.length; i++) {
                ...
                // 内容不存储还原，分词
                doc.add(new Field("content", MockDataUtil.CONTENTS[i], Field.Store.YES, Field.Index.ANALYZED));
                //
                ...
                // 加权
                String et = MockDataUtil.EMAILS[i].substring(MockDataUtil.EMAILS[i].indexOf("@") + 1);
                // 默认是1.0 越高权值越高
